...
import com.qa.voltarengel.pages.voltarengelcaregiverdepressionPage;

public class voltarengelcaregiverdepressionTest extends BaseClass{
	
	voltarengelcaregiverdepressionPage vn;
	...
	private void launchingTheBrowser() throws IOException, Exception {
	 ...
	 @BeforeMethod
		...
		vn=new voltarengelcaregiverdepressionPage();
	        switchingTab(firstTab);
	        launchURL(loadConfig("voltarengel_Config.properties").getProperty("preProdvoltarengelcaregiverdepressionUrl"));
			switchingTab(secondTab);
			launchURL(loadConfig("voltarengel_Config.properties").getProperty("LivevoltarengelcaregiverdepressionUrl"));
		 }
	 ...
	 @Test(priority = 3, enabled = true)
	    private void VerifyCaregiverDepression_Breadcrumbs_Button() throws Exception {
		     switchingTab(firstTab);
	         vn.clickonCaregiverDepression_Breadcrumbs();
	         String preProdUrl = compareUrl(excelRead("Voltarengel", 1, 0));
	         switchingTab(secondTab);
	         vn.clickonCaregiverDepression_Breadcrumbs();
	         String liveUrl = compareUrl(excelRead("Voltarengel", 1, 1));
	         boolean compareTwoText = compareTwoText(preProdUrl, liveUrl);
	         System.out.println("Preprod URL and Live URL are same for using Caregiver Depression_Breadcrumbs:"+compareTwoText);
	    }												
