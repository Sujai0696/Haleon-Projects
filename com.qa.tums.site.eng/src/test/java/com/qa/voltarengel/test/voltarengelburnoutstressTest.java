...
import com.qa.voltarengel.pages.voltarengelburnoutstressPage;

public class voltarengelburnoutstressTest extends BaseClass{
	
	voltarengelburnoutstressPage vn;
	...
	private void launchingTheBrowser() throws IOException, Exception {
	 ...
	 @BeforeMethod
		...
		vn=new voltarengelburnoutstressPage();
	        switchingTab(firstTab);
	        launchURL(loadConfig("voltarengel_Config.properties").getProperty("preProdvoltarengelcaregiverburnoutstressUrl"));
			switchingTab(secondTab);
			launchURL(loadConfig("voltarengel_Config.properties").getProperty("LivevoltarengelcaregiverburnoutstressUrl"));
		 }
	 ...
	 @Test(priority = 2, enabled = true)
	    private void VerifyCarewalkssupport_breadcrumbs_Button() throws Exception {
		     switchingTab(firstTab);
	         vn.clickonCarewalkssupportResource_Breadcrumbs();
	         String preProdUrl = compareUrl(excelRead("Voltarengel", 1, 0));
	         switchingTab(secondTab);
	         vn.clickonCarewalkssupportResource_Breadcrumbs();
	         String liveUrl = compareUrl(excelRead("Voltarengel", 1, 1));
	         boolean compareTwoText = compareTwoText(preProdUrl, liveUrl);
	         System.out.println("Preprod URL and Live URL are same for using Carewalks support_Breadcrumbs:"+compareTwoText);
	    }														
	 @Test(priority = 3, enabled = true)
	    private void VerifyCaregiverburnout_Breadcrumbs_Button() throws Exception {
		     switchingTab(firstTab);
	         vn.clickoncaregiverburnout_Breadcrumbs();
	         String preProdUrl = compareUrl(excelRead("Voltarengel", 1, 0));
	         switchingTab(secondTab);
	         vn.clickoncaregiverburnout_Breadcrumbs();
	         String liveUrl = compareUrl(excelRead("Voltarengel", 1, 1));
	         boolean compareTwoText = compareTwoText(preProdUrl, liveUrl);
	         System.out.println("Preprod URL and Live URL are same for using Care giver burn out_Breadcrumbs:"+compareTwoText);
	    }														
	 @Test(priority = 4, enabled = true)
	    private void VerifyShowReferences_Button() throws Exception {
		     switchingTab(firstTab);
	         vn.clickonShowReferences();
	         System.out.println("Verified Show References displayed on Preprod Url");
	         switchingTab(secondTab);
	         vn.clickonShowReferences();
