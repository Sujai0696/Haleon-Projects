...
import com.qa.voltarengel.pages.voltarengelwhatiscaregiverPage;

public class voltarengelwhatiscaregiverTest extends  BaseClass{
	
	voltarengelwhatiscaregiverPage vn;
	...
	private void launchingTheBrowser() throws IOException, Exception {
	 ...
	 @BeforeMethod
		...
		vn=new voltarengelwhatiscaregiverPage();
	        switchingTab(firstTab);
	        launchURL(loadConfig("voltarengel_Config.properties").getProperty("preProdvoltarengelwhatiscaregiverUrl"));
			switchingTab(secondTab);
			launchURL(loadConfig("voltarengel_Config.properties").getProperty("LivevoltarengelwhatiscaregiverUrl"));
		 }
	 ...
	 @Test(priority = 3, enabled = true)
	    private void VerifyWhatiscaregiver_Breadcrumbs_Button() throws Exception {
		     switchingTab(firstTab);
	         vn.clickonwhatiscaregiver_Breadcrumbs();
	         String preProdUrl = compareUrl(excelRead("Voltarengel", 1, 0));
	         switchingTab(secondTab);
	         vn.clickonwhatiscaregiver_Breadcrumbs();
	         String liveUrl = compareUrl(excelRead("Voltarengel", 1, 1));
	         boolean compareTwoText = compareTwoText(preProdUrl, liveUrl);
	         System.out.println("Preprod URL and Live URL are same for using What is caregiver_Breadcrumbs:"+compareTwoText);
	    }													
