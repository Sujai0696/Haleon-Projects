...
import com.qa.voltarengel.pages.voltarengelcaregiverfatiguePage;


public class voltarengelcaregiverfatigueTest extends BaseClass{
	
	voltarengelcaregiverfatiguePage vn;
	...
	private void launchingTheBrowser() throws IOException, Exception {
	 ...
	 @BeforeMethod
		...
		vn=new voltarengelcaregiverfatiguePage();
	        switchingTab(firstTab);
	        launchURL(loadConfig("voltarengel_Config.properties").getProperty("preProdVoltarengelcaregiverfatigueUrl"));
			switchingTab(secondTab);
			launchURL(loadConfig("voltarengel_Config.properties").getProperty("LiveVoltarengelcaregiverfatigueUrl"));
		 }
	 ...
	 @Test(priority = 3, enabled = true)
	    private void Verifycaregiverfatigue_breadcrumbs_Button() throws Exception {
		     switchingTab(firstTab);
	         vn.clickoncarewgiverfatigue_Breadcrumbs();
	         String preProdUrl = compareUrl(excelRead("Voltarengel", 1, 0));
	         switchingTab(secondTab);
	         vn.clickoncarewgiverfatigue_Breadcrumbs();
	         String liveUrl = compareUrl(excelRead("Voltarengel", 1, 1));
	         boolean compareTwoText = compareTwoText(preProdUrl, liveUrl);
	         System.out.println("Preprod URL and Live URL are same for using Care giver Fatigue Breadcrumbs:"+compareTwoText);
	    }										
	 @Test(priority = 4, enabled = true)
	    private void VerifyshowReferences_Button() throws Exception {
		     switchingTab(firstTab);
	         vn.clickonShowReferences();
	         System.out.println("Verified the Show References is clicable on Preprod Url");
	         switchingTab(secondTab);
	         vn.clickonShowReferences();
	         System.out.println("Verified the Show References is clicable on Live Url");
	    }										
