...
import com.qa.voltarengel.pages.voltarengelcaregiverselfcaretipsPage;

public class voltarengelcaregiverselfcaretipsTest extends BaseClass{
	
	voltarengelcaregiverselfcaretipsPage vn;
	...
	private void launchingTheBrowser() throws IOException, Exception {
	 ...
	 @BeforeMethod
		...
		vn=new voltarengelcaregiverselfcaretipsPage();
	        switchingTab(firstTab);
	        launchURL(loadConfig("voltarengel_Config.properties").getProperty("preProdVoltarengelcaregiverselfcaretipsUrl"));
			switchingTab(secondTab);
			launchURL(loadConfig("voltarengel_Config.properties").getProperty("LiveVoltarengelcaregiverselfcaretipsUrl"));
		 }
	 ...
	 @Test(priority = 3, enabled = true)
	    private void Verifyselfcaretips_breadcrumbs_Button() throws Exception {
		     switchingTab(firstTab);
	         vn.clickonselfcaretips_Breadcrumbs();
	         String preProdUrl = compareUrl(excelRead("Voltarengel", 1, 0));
	         switchingTab(secondTab);
	         vn.clickonselfcaretips_Breadcrumbs();
	         String liveUrl = compareUrl(excelRead("Voltarengel", 1, 1));
	         boolean compareTwoText = compareTwoText(preProdUrl, liveUrl);
	         System.out.println("Preprod URL and Live URL are same for using selfcare tips_Breadcrumbs:"+compareTwoText);
	    }								
