...
import com.qa.voltarengel.pages.voltarengelkneestrechesarthristPage;


public class voltarengelkneestrechesarthristTest extends BaseClass{
	
	voltarengelkneestrechesarthristPage vn;
	...
	private void launchingTheBrowser() throws IOException, Exception {
	 ...
	 @BeforeMethod
		...
		vn=new voltarengelkneestrechesarthristPage();
	        switchingTab(firstTab);
	        launchURL(loadConfig("voltarengel_Config.properties").getProperty("preProdvoltarengelkneestrechesarthristUrl"));
			switchingTab(secondTab);
			launchURL(loadConfig("voltarengel_Config.properties").getProperty("LivevoltarengelkneestrechesarthristUrl"));
		 }
	 ...
	 @Test(priority = 4, enabled = true)
	    private void VerifyStandingstrechespain_Breadcrumbs_Button() throws Exception {
		     switchingTab(firstTab);
	         vn.clickonstandingstrechespain_Breadcrumbs();
	         String preProdUrl = compareUrl(excelRead("Voltarengel", 1, 0));
	         switchingTab(secondTab);
	         vn.clickonstandingstrechespain_Breadcrumbs();
	         String liveUrl = compareUrl(excelRead("Voltarengel", 1, 1));
	         boolean compareTwoText = compareTwoText(preProdUrl, liveUrl);
	         System.out.println("Preprod URL and Live URL are same for using Standing streches pain_Breadcrumbs:"+compareTwoText);
	    }					
	 @Test(priority = 5, enabled = true)
	    private void VerifyShowReferences_Button() throws Exception {
		     switchingTab(firstTab);
	         vn.clickonShowReferences();
	         System.out.println("Verified Show References is displayed on Preprod Url");
	         switchingTab(secondTab);
	         vn.clickonShowReferences();
	         System.out.println("Verified Show References is displayed on Live Url");
	 }
	 @Test(priority = 5, enabled = true)
