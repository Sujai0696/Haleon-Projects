...
import com.qa.voltarengel.pages.voltarengelarthristdiagnosedPage;

public class voltarengelarthristdiagnosedTest extends BaseClass{
	
	voltarengelarthristdiagnosedPage vn;
	...
	private void launchingTheBrowser() throws IOException, Exception {
	 ...
	 @BeforeMethod
		...
		vn=new voltarengelarthristdiagnosedPage();
	        switchingTab(firstTab);
	        launchURL(loadConfig("voltarengel_Config.properties").getProperty("preProdvoltarengelArthristDiagnosedUrl"));
			switchingTab(secondTab);
			launchURL(loadConfig("voltarengel_Config.properties").getProperty("LivevoltarengelArthristDiagnosedUrl"));
		 }
	 ...
	 @Test(priority = 3, enabled = true)
	    private void VerifyArthristDiagnosed_breadcrumbs_Button() throws Exception {
		     switchingTab(firstTab);
	         vn.clickonArthristDiagnosed_Breadcrumbs();
	         String preProdUrl = compareUrl(excelRead("Voltarengel", 1, 0));
	         switchingTab(secondTab);
	         vn.clickonArthristDiagnosed_Breadcrumbs();
	         String liveUrl = compareUrl(excelRead("Voltarengel", 1, 1));
	         boolean compareTwoText = compareTwoText(preProdUrl, liveUrl);
	         System.out.println("Preprod URL and Live URL are same for using Arthrist Diagnosed_Breadcrumbs:"+compareTwoText);
	    }							
	 @Test(priority = 3, enabled = true)
	    private void VerifySymptomsArthrist_Button() throws Exception {
		     switchingTab(firstTab);
	         vn.clickonSymptomsArthrist();
	         String preProdUrl = compareUrl(excelRead("Voltarengel", 1, 0));
	         switchingTab(secondTab);
	         vn.clickonSymptomsArthrist();
	         String liveUrl = compareUrl(excelRead("Voltarengel", 1, 1));
	         boolean compareTwoText = compareTwoText(preProdUrl, liveUrl);
	         System.out.println("Preprod URL and Live URL are same for using Symptoms Arthrist:"+compareTwoText);
	    }					
