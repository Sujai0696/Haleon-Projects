...
import com.qa.rinazina.it.pages.RinazinaRaffreddorePage;

public class RinazinaRaffreddoreTest extends BaseClass{
	
	RinazinaRaffreddorePage rr;
	...
	private void launchingTheBrowser() throws IOException, Exception {
	 ...
	 @BeforeMethod
	    private void launchingTheUrl() throws IOException, Exception {
		 rr=new RinazinaRaffreddorePage();
	        switchingTab(firstTab);
	        launchURL(loadConfig("rinazina_itConfig.properties").getProperty("preProdRinazinaRaffreddoreUrl"));
			switchingTab(secondTab);
			launchURL(loadConfig("rinazina_itConfig.properties").getProperty("LiveRinazinaRaffreddoreUrl"));
		 }	
	 ...
	 @Test(priority=3,enabled=true)
		public void verifyRinazinaRaffreddoreBreadcrumbsLink() throws Throwable
		{
		 switchingTab(firstTab);
	        rr.clickonRaffreddoreBreadcrumbs();
	        String preProdUrl = compareUrl(excelRead("Rinazina_it", 1, 0));
	        switchingTab(secondTab);
	        rr.clickonRaffreddoreBreadcrumbs();
	        String liveUrl = compareUrl(excelRead("Rinazina_it", 1, 1));
	        boolean compareTwoText = compareTwoText(preProdUrl, liveUrl);
	        System.out.println("Preprod URL and Live URL are same for RinazinaRaffreddoreBreadcrumbs:"+compareTwoText);
		}			
	 ...
	 @Test(priority=4,enabled=true)
		public void verifyRinazinapersaperenedipiuLink() throws Throwable
		{
		 switchingTab(firstTab);
	        ...
	        String liveUrl = compareUrl(excelRead("Rinazina_it", 1, 1));
	        boolean compareTwoText = compareTwoText(preProdUrl, liveUrl);
	        System.out.println("Preprod URL and Live URL are same for Rinazinapersaperenedipiu:"+compareTwoText);
		}			
