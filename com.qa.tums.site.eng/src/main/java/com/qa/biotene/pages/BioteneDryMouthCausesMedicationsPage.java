...
public class BioteneDryMouthCausesMedicationsPage extends BaseClass{
	
	public BioteneDryMouthCausesMedicationsPage() {
	...
	@FindBy(xpath = "//li[@class='breadcrumb-list-item odd last is-current ']//ancestor::a[text()='MEDICATIONS & DRY MOUTH']")
	private static WebElement breadcrumbThree;
	
	@FindBy(xpath = "//video[@class='custom-video-player']")
	private static WebElement videoPlay;
	
	@FindBy(xpath = "//video[@class='custom-video-player']//self::video[@data-gtm-play-sent='true']")
	private static WebElement videoStreaming;
	
	@FindBy(xpath = "//video[@class='custom-video-player']//self::video[@data-gtm-progress='0.25']")
	...
	public void clickVideo() throws Exception{
		waitForPageLoadJava();
		if (isElementPresent(videoPlay)) {
			scrollDownUsingElement(videoPlay);
			moveToElement(videoPlay);
			actionClick(videoPlay);
			Thread.sleep(5000);
			visibilityOf(videoStreaming);
			Assert.assertTrue(elementIsDisplayed(videoStreaming));
			moveToElement(videoPlay);
			actionClick(videoPlay);
