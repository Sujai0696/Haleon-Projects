...
public class BioteneMouthwashPage extends BaseClass{
	
	public BioteneMouthwashPage() {
	...
	@FindBy(xpath = "//li[@class='breadcrumb-list-item odd last is-current ']//ancestor::a[text()='Biotène Dry Mouth Oral Rinse (Mouthwash) ']")
	private static WebElement breadcrumbThree;
	
	@FindBy(xpath = "//span[text()='BUY NOW']//preceding::div[@class='gtm-buynow']")
	private static WebElement buyNow;
	
	@FindBy(xpath = "//h2[text()='Biotene Oral Rinse 16 oz']")
	...
	@FindBy(xpath = "//div[@data-bv-seo='true']//button[@id='WAR']")
	private static WebElement writeARivew;
	
	@FindBy(xpath = "//span[text()='My Review for Dry Mouth Oral Rinse']")
	private static WebElement writeARivewText;
	
	@FindBy(xpath = "//button[@name='Cancel']")
	private static WebElement writeARivewClose;
	
	@FindBy(xpath = "//div[@class='bv-mbox-overlay bv-hiding bv-mbox-overlay-hide']")
	private static WebElement writeARivewVerify;
	
	@FindBy(xpath = "//*[@href='/what-is-dry-mouth/recognizing-dry-mouth/']//self::a[text()='dry mouth']")
	private static WebElement dryMouthLink;
	
	@FindBy(xpath = "//*[@href='/dry-mouth-health-impacts/understanding-dry-mouth/']//self::a[text()='explore resources']")
	private static WebElement exploreResourcesLink;
	
	@FindBy(xpath = "//*[@href='/dry-mouth-products/']//self::a[text()='Biotène site']")
	private static WebElement bioteneSiteLink;
	
	@FindBy(xpath = "//li[@class='carousel-nav-item is-active']//ancestor::a[text()='1']")
...
//	//div[@class='carousel component section home-carousel-products even grid_12 alpha omega initialized']//a[@role='button'][normalize-space()='2']
//	//li[@class='carousel-nav-item ']//a[@role='button'][normalize-space()='2']
	...
	public void clickBannerWriteAReview() throws Exception{
		waitForPageLoadJava();
		scrollDownUsingElement(writeARivew);
		clickElementUsingJavaScript(writeARivew);
		if (isElementPresent(writeARivewText)) {
			visibilityOf(writeARivewClose);
			clickElementUsingJavaScript(writeARivewClose);
			visibilityOf(writeARivewVerify);
			Assert.assertTrue(elementIsDisplayed(writeARivewVerify));
			System.out.println("Banner Write a review is verified successfully");
		} else {
			System.out.println("Banner Write a review  button is not clicked or Buy Now text is not displayed in the page");
			throw new Exception("Banner Write a review  is not clicked or Buy Now text is not displayed in the page");
		}
	}
	
	public void clickDryMouthLink() throws Exception{
		waitForPageLoadJava();
		if (isElementPresent(dryMouthLink)) {
			scrollDownUsingElement(dryMouthLink);
			clickElementUsingJavaScript(dryMouthLink);
			Thread.sleep(1000);
			System.out.println("Dry mouth link is clicked successfully");
		} else {
			System.out.println("Dry mouth link is not presented in the page");
			throw new Exception("Dry mouth link is not presented in the page");
		}
	}
	
	public void clickExploreResourcesLink() throws Exception{
		waitForPageLoadJava();
		if (isElementPresent(exploreResourcesLink)) {
			scrollDownUsingElement(exploreResourcesLink);
			clickElementUsingJavaScript(exploreResourcesLink);
			Thread.sleep(1000);
			System.out.println("Explore Resources link is clicked successfully");
		} else {
			System.out.println("Explore Resources link is not presented in the page");
			throw new Exception("Explore Resources link is not presented in the page");
		}
	}
	
	public void clickBioteneSiteLink() throws Exception{
		waitForPageLoadJava();
		if (isElementPresent(bioteneSiteLink)) {
			scrollDownUsingElement(bioteneSiteLink);
			clickElementUsingJavaScript(bioteneSiteLink);
			Thread.sleep(1000);
			System.out.println("Biotene Site link is clicked successfully");
		} else {
			System.out.println("Biotene Site link is not presented in the page");
			throw new Exception("Biotene Site link is not presented in the page");
		}
	}
