...
public class PrepHointmentapplicatorPage extends BaseClass{
	
	public PrepHointmentapplicatorPage() {
	...
	@FindBy(xpath = "//li[text()=' How to Apply PREPARATION H Ointment with an Applicator ']")
	private static WebElement ointmentapplicator_Breadcrumbs;
	
	@FindBy(xpath = "//h1[text()='How to Apply Ointment with an Applicator']")
	private static WebElement ointmentapplicator_txt_Breadcrumbs;
	
	@FindBy(xpath = "//img[@alt='How to Apply Ointment with an Applicator']")
	...
	@FindBy(xpath = "//a[text()='Hemorrhoids and Physical Activity']")
	private static WebElement hemorrhoidactivity;
	
	@FindBy(xpath = "//h1[text()='How Does Exercise Help Hemorrhoids?']")
	private static WebElement hemorrhoidactivity_txt;
	...
	//Methods//
		...
		public void clickonCookieBtn() throws Exception {
			...
			public void clickonOintmentApplicator_Breadcrumbs() throws Exception{
		    	waitForPageLoadJava();
				moveToElement(ointmentapplicator_Breadcrumbs);
				if (isElementPresent(ointmentapplicator_Breadcrumbs)) {
					visibilityOf(ointmentapplicator_txt_Breadcrumbs);
					boolean elementIsDisplayed=elementIsDisplayed(ointmentapplicator_txt_Breadcrumbs);
					Assert.assertTrue(elementIsDisplayed(ointmentapplicator_txt_Breadcrumbs));
					pageLoad();
				} else {
					throw new Exception("ointmentapplicator_txt_Breadcrumbs are not Present on Page");
				}
			}
			...
			public void clickonHemorroidActivity() throws Exception{
		    	waitForPageLoadJava();
		    	scrollDownUsingElement(hemorrhoidactivity);
				moveToElement(hemorrhoidactivity);
				if (isElementPresent(hemorrhoidactivity)) {
					clickElementUsingJavaScript(hemorrhoidactivity);
					visibilityOf(hemorrhoidactivity_txt);
					boolean elementIsDisplayed=elementIsDisplayed(hemorrhoidactivity_txt);
					Assert.assertTrue(elementIsDisplayed(hemorrhoidactivity_txt));
					System.out.println(getElementText(hemorrhoidactivity_txt));
					pageLoad();
				} else {
					throw new Exception("hemorrhoidactivity_txt are not Present on Page");
				}
			}
