...
public class PrepHdontfeelremorsePage extends BaseClass{
	
	public PrepHdontfeelremorsePage() {
	...
	private static WebElement dontfeelremorse_Breadcrumbs;	
	
	@FindBy(xpath = "//div[@class='richText component section p-d-b-2 p-d-r-5 p-m-b-1 head-alt first odd col-xs-12']")
	private static WebElement dontfeelremorse_txt_Breadcrumbs;	
	...
	@FindBy(xpath = "//div[@class='ytp-cued-thumbnail-overlay']//child::div[@class='ytp-cued-thumbnail-overlay-image']")
	...
	@FindBy(xpath = "//div[@class='richText component section p-d-b-2 p-d-r-5 p-m-b-1 head-alt first odd col-xs-12']//h1[text()='Applying Maximum Strength Pain Relief Cream']")
	...
	//Methods//
			...
			public void clickonCookieBtn() throws Exception {
				...
				public void clickonDontfeelremorse_Breadcrumbs() throws Exception{
			    	waitForPageLoadJava();
					moveToElement(dontfeelremorse_Breadcrumbs);
					if (isElementPresent(dontfeelremorse_Breadcrumbs)) {
						visibilityOf(dontfeelremorse_txt_Breadcrumbs);
						boolean elementIsDisplayed=elementIsDisplayed(dontfeelremorse_txt_Breadcrumbs);
						Assert.assertTrue(elementIsDisplayed(dontfeelremorse_txt_Breadcrumbs));
						pageLoad();
					} else {
						throw new Exception("dontfeelremorse_txt_Breadcrumbs are not Present on Page");
					}
					
				}			
				public void clickonVideodontfeelremorse() throws Exception{
			    	waitForPageLoadJava();
					...
					if (isElementPresent(videobtn)) {
						...
						//clickElementUsingJavaScript(videodispaly_button);
						implicitWait();
