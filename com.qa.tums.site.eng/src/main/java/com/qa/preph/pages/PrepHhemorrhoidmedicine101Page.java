...
public class PrepHhemorrhoidmedicine101Page extends BaseClass{
	
	public PrepHhemorrhoidmedicine101Page() {
	...
	@FindBy(xpath = "//li[text()=' Hemorrhoid Medicine 101 ']")
	private static WebElement hemorrhoidmedicine_Breadcrumbs;	
	
	
	@FindBy(xpath = "//h1[text()='Hemorrhoid Medicine 101']")
	private static WebElement hemorrhoidmedicine_txt_Breadcrumbs;	
	...
	//Methods//
		...
		public void clickonCookieBtn() throws Exception {
			...
			public void clickonHemorrhoidMedicine101Breadcrumbs() throws Exception{
		    	waitForPageLoadJava();
				moveToElement(hemorrhoidmedicine_Breadcrumbs);
				if (isElementPresent(hemorrhoidmedicine_Breadcrumbs)) {
					
					visibilityOf(hemorrhoidmedicine_txt_Breadcrumbs);
					boolean elementIsDisplayed=elementIsDisplayed(hemorrhoidmedicine_txt_Breadcrumbs);
					Assert.assertTrue(elementIsDisplayed(hemorrhoidmedicine_txt_Breadcrumbs));
					System.out.println(getElementText(hemorrhoidmedicine_txt_Breadcrumbs));
					pageLoad();
				} else {
					throw new Exception("hemorrhoidmedicine_txt_Breadcrumbs are not Present on Page");
				}
