...
public class PrepHHomeRemedieshemorrhoidPage extends BaseClass{
	
	
	public PrepHHomeRemedieshemorrhoidPage() {
	...
	@FindBy(xpath = "//li[text()=' Home Remedies for Hemorrhoids ']")
	private static WebElement homeremediesHemorrhoids_Breadcrumbs;
	
	@FindBy(xpath = "//h1[text()='Common Home Remedies for Hemorrhoids: How to Treat Hemorrhoids Naturally']")
	private static WebElement homeremediesHemorrhoids_txt_Breadcrumbs;
	...
	//Methods//
		...
		public void clickonCookieBtn() throws Exception {
			...
			public void clickonHomeremediesHemorrhoidsBreadcrumbs() throws Exception{
		    	waitForPageLoadJava();
				moveToElement(homeremediesHemorrhoids_Breadcrumbs);
				if (isElementPresent(homeremediesHemorrhoids_Breadcrumbs)) {
					
					visibilityOf(homeremediesHemorrhoids_Breadcrumbs);
					boolean elementIsDisplayed=elementIsDisplayed(homeremediesHemorrhoids_Breadcrumbs);
					Assert.assertTrue(elementIsDisplayed(homeremediesHemorrhoids_Breadcrumbs));
					System.out.println(getElementText(homeremediesHemorrhoids_txt_Breadcrumbs));
					pageLoad();
				} else {
					throw new Exception("homeremediesHemorrhoids_Breadcrumbs are not Present on Page");
				}
