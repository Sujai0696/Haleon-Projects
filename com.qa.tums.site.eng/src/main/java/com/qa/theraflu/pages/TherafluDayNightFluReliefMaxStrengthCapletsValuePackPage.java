...
public class TherafluDayNightFluReliefMaxStrengthCapletsValuePackPage extends BaseClass{
	
	public TherafluDayNightFluReliefMaxStrengthCapletsValuePackPage() {
	...
	@FindBy(xpath = "//li[@class='breadcrumb-list-item odd last is-current ']//ancestor::a[text()='Day/Night Flu Relief Max Strength Caplets Value Pack']")
	...
	@FindBy(xpath = "//span[text()='My Review for THERAFLU MAX STRGTH RELIEF DT/NT 2X20CT']")
...
//			clickElementUsingJavaScript(breadcrumbTwo);
	...
	public void clickImgCarousel() throws Exception{
		...
		if (isElementPresent(imgActiveOne)) {
			...
			visibilityOf(imgActiveOne);
			Assert.assertTrue(elementIsDisplayed(imgActiveOne));
