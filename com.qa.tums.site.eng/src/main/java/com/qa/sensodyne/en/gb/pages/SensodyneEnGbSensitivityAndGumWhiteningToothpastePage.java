...
public class SensodyneEnGbSensitivityAndGumWhiteningToothpastePage extends BaseClass{
	
	public SensodyneEnGbSensitivityAndGumWhiteningToothpastePage() {
	...
	@FindBy(xpath = "//div[@class='breadcrumbs component section breadcrumbs-comp container-override first odd last col-xs-12']//ancestor::a[text()='Home']//self::a[@href='/en-gb/']")
	private static WebElement breadcrumbOne;
	
	@FindBy(xpath = "//div[@class='breadcrumbs component section breadcrumbs-comp container-override first odd last col-xs-12']//ancestor::a[text()='Our Products']//self::a[@href='/en-gb/products/']")
	private static WebElement breadcrumbTwo;
	
	@FindBy(xpath = "//div[@class='breadcrumbs component section breadcrumbs-comp container-override first odd last col-xs-12']//ancestor::a[text()='Sensitivity & Gum whitening toothpaste']//self::a[@href='/en-gb/products/sensitivity-and-gum-whitening-toothpaste/']")
	private static WebElement breadcrumbThree;
	
	@FindBy(xpath = "//span[@class='ps-button-label' and text()='Buy Now']//ancestor::a")
	private static WebElement buyNow;
	
	@FindBy(xpath = "//h2[@class='ps-product-name' and text()='Sensodyne Sensitivity and Gum Whitening Toothpaste']")
	private static WebElement buyNowText;
	
	@FindBy(xpath = "//span[@class='ps-lightbox-close']")
	private static WebElement buyNowClose;
	
	@FindBy(xpath = "//div[@aria-label='Shop Sensodyne Sensitivity and Gum Whitening Toothpaste from other retailers dialog box/popup.' and @style='display: none; z-index: -10000; opacity: 0;']")
	private static WebElement buyNowCloseVerify;
	
	@FindBy(xpath = "//a[@href='/en-gb/products/sensitivity-and-gum-toothpaste/']//following::img[@alt='Sensodyne Sensitivity and Gum Toothpaste Pack']")
	private static WebElement availableProductImg;
	
	@FindBy(xpath = "//span[@class='light-blue-txt-light-18-roboto' and text()='Sensitivity & Gum']//ancestor::a[@href='/en-gb/products/sensitivity-and-gum-toothpaste/']")
	private static WebElement availableProductLink;
	...
	@FindBy(xpath = "//span[text()='Most Popular Products']//following::a[text()='Repair & Protect Toothbrush']//self::a[@href='/en-gb/products/repair-and-protect-toothbrush/']")
...
//	        Assert.assertFalse(hasValidUrl);
	...
	public void clickAvailableProductImg() throws Exception{
		waitForPageLoadJava();
		if (isElementPresent(availableProductImg)) {
			scrollDownUsingElement(availableProductImg);
			clickElementUsingJavaScript(availableProductImg);
			pageLoad();
		} else {
			System.out.println("Available Product Img is not presented in the page");
			throw new Exception("Available Product Img is not presented in the page");
		}
	}
	
	public void clickAvailableProductLink() throws Exception{
		waitForPageLoadJava();
		if (isElementPresent(availableProductLink)) {
			scrollDownUsingElement(availableProductLink);
			clickElementUsingJavaScript(availableProductLink);
			pageLoad();
		} else {
			System.out.println("Available Product Link is not presented in the page");
			throw new Exception("Available Product Link is not presented in the page");
		}
	}
