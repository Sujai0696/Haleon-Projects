...
public class CentrumVitaminB6Page extends BaseClass {
	...
	@FindBy(xpath = "//strong[text()='Heart Health']//ancestor::div[contains(@class,'box component section bg-color-white rounded')]")
	private static WebElement heartHealth;

	@FindBy(xpath = "//strong[text()='Energy']//ancestor::div[contains(@class,'box component section bg-color-white rounded')]")
	private static WebElement energy;

	@FindBy(xpath = "//strong[text()='Brain Health']//ancestor::div[contains(@class,'box component section bg-color-white rounded')]")
	...
	@FindBy(xpath = "//li[@class='breadcrumb-list-item even last is-current ']//ancestor::a[text()='Vitamin B6']")
	...
	public void clickProductMultiGummiesMen() throws Exception {
		...
		scrollDownUsingElement(featuredProducts);
		clickElementUsingJavaScript(featuredProducts);
		if (isElementPresent(productMultiGummiesMen)) {
			scrollDownUsingElement(productMultiGummiesMen);
			clickElementUsingJavaScript(productMultiGummiesMen);
		} else {
	...
	public void clickProductSilverWomen() throws Exception {
		...
		scrollDownUsingElement(featuredProducts);
		clickElementUsingJavaScript(featuredProducts);
		if (isElementPresent(productSilverWomen)) {
			scrollDownUsingElement(productSilverWomen);
			clickElementUsingJavaScript(productSilverWomen);
		} else {
	...
	public void clickProductWomensMultivitamin() throws Exception {
		...
		scrollDownUsingElement(featuredProducts);
		clickElementUsingJavaScript(featuredProducts);
		if (isElementPresent(productWomensMultivitamin)) {
			scrollDownUsingElement(productWomensMultivitamin);
			clickElementUsingJavaScript(productWomensMultivitamin);
		} else {
